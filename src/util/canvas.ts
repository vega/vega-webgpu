export function blend(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, item: {blend: string}) {
  ctx.globalCompositeOperation = item.blend || 'source-over';
}

export function isGradient(value: {gradient?: string}) {
