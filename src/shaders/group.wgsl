...
fn main_vertex([[location(0)]] position : vec2<f32>) -> VertexOutput {
    ...
    var pos : vec2<f32> = position * uniforms.scale;
    pos = pos + uniforms.origin;
...
fn main_fragment([[location(0)]] uv : vec2<f32>) -> [[location(0)]] vec4<f32> {
    ...
    var sw : vec2<f32> = vec2<f32>(colors.strokewidth, colors.strokewidth) * 2.0 / uniforms.resolution;
